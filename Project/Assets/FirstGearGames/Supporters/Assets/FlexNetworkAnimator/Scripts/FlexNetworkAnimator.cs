    ...
    public class FlexNetworkAnimator : NetworkBehaviour
    {
        ...
        bool SendMessagesAllowed
        {
            get
            {
                ...
                return (base.hasAuthority && _clientAuthoritative);
            }
        }
        ...
        private bool SkipAnimationData()
        {
            /* If owner and client authority, already ran locally.
            * 
            * If not synchronize to owner, and has authority then ignore. */
            if ((_clientAuthoritative && base.hasAuthority) || (!_synchronizeToOwner && base.hasAuthority))
                return true;
            else
                return false;
        }
